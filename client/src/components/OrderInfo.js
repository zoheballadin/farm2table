...
<div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
  ...
  <div class="flex justify-start item-start space-y-2 flex-col">
    ...
    <p class="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">{order.createdAt}</p>
  </div>
  <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
    <div class="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
      <div class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
        <p class="text-lg md:text-xl dark:text-white font-semibold leading-6 xl:leading-5 text-gray-800">Your Order</p>
        
        <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
          <div class="pb-4 md:pb-8 w-full md:w-40">
            <img class="w-full hidden md:block" src={order.product.imageUrl} alt="dress" />
          ...
          <div class="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
            <div class="w-full flex flex-col justify-start items-start space-y-8">
              ...
              <div class="flex justify-start items-start flex-col space-y-2">
                <p class="text-lg dark:text-white leading-none text-gray-800"><span class="dark:text-gray-400 text-gray-300">Category: </span> {order.product.category}</p>
                <p class="text-md dark:text-white leading-none text-gray-800"><span class="dark:text-gray-400 text-gray-300 ">Description: </span> {order.product.description}</p>
            ...
            <div class="flex justify-between space-x-8 items-start w-full">
              <p class="text-base dark:text-white xl:text-lg leading-6">Rs. {order.price} </p>
              <p class="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{order.qty} kg</p>
              <p class="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">Rs. {order.total}</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
          ...
          <div class="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
            <div class="flex justify-between w-full">
              <p class="text-base dark:text-white leading-4 text-gray-800">Price per piece</p>
              <p class="text-base dark:text-gray-300 leading-4 text-gray-600">Rs. {order.price}</p>
            </div>
            <div class="flex justify-between items-center w-full">
              <p class="text-base dark:text-white leading-4 text-gray-800">Quantity </p>
              <p class="text-base dark:text-gray-300 leading-4 text-gray-600">{order.qty} kg</p>
            </div>
            <div class="flex justify-between items-center w-full">
          ...
          <div class="flex justify-between items-center w-full">
            ...
            <p class="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">Rs. {order.total}</p>
          </div>
        </div>
    ...
    <div class="bg-gray-50 dark:bg-gray-800 w-full xl:w-96 flex justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col">
      <h3 class="text-xl dark:text-white font-semibold leading-5 text-gray-800">Vendor</h3>
      <div class="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
        <div class="flex flex-col justify-start items-start flex-shrink-0">
          <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
            <img className='w-16' src="https://kingstonplaza.com/wp-content/uploads/2015/07/generic-avatar.png" alt="avatar" />
            <div class="flex justify-start items-start flex-col space-y-2">
              <p class="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">{order.seller.fullname}</p>
              <p class="text-sm dark:text-gray-300 leading-5 text-gray-600">joined {order.seller.createdAt}</p>
            </div>
          </div>
          ...
          <div class="flex justify-center text-gray-800 dark:text-white md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
            ...
            <p class="cursor-pointer text-sm leading-5 ">{order.seller.email}</p>
          </div>
        </div>
        <div class="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
          <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
            <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
              ...
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{order.seller.address}</p>
            </div>
            <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
              <p class="text-base dark:text-white font-semibold leading-4 text-center md:text-left text-gray-800">Mobile</p>
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{order.seller.phone}</p>
            </div>
            <div class="flex mb-8 justify-center md:justify-start items-center md:items-start flex-col space-y-4">
              ...
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 mb-8 text-center md:text-left text-sm leading-5 text-gray-600">{order.seller.address}</p>
            </div>
          </div>
